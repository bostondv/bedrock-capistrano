...
# This task compiles and uploads the theme files to the server
...
namespace :deploy do
  ...
  task :copy do
    on roles(:web) do
      ...
      info " Local dist path: #{fetch(:local_dist_path)} "
      info " Remote dist path: #{fetch(:remote_dist_path)} "
